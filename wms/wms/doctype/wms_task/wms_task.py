...
class WMSTask(Document):
    def validate(self):
        ...
        elif self.date_of_completion and getdate(self.date_of_completion) <= getdate(
            self.due_date
        ):
            self.status = "Ontime"
        elif self.date_of_completion and getdate(self.date_of_completion) > getdate(
        ...
        if self.get("__islocal"):
            ...
            def validate_holiday_leave(self):
                ...
                if get_leave(self.assign_to, self.due_date):
                    frappe.msgprint(
                        f"{self.assign_to} Applied For Leave On {self.due_date}.So Due Date Adjust As per next working days "
                    )
                    self.due_date = add_days(self.due_date, 1)
                    return validate_holiday_leave(self)
                if get_holidays(self.assign_to, self.due_date):
                    frappe.msgprint(
                        f"Public Holiday On {self.due_date}. So Due Date Adjust As per next working days "
                    )
                    self.due_date = add_days(self.due_date, 1)
                    return validate_holiday_leave(self)
...
def get_users(doctype, txt, searchfield, start, page_len, filters):
    ...
    reporting_employees = frappe.get_all(
        "Employee", filters={"reports_to": emp_id}, fields=["name"]
    )
    for row in reporting_employees:
        employees.append(row.name)
    if len(employees) >= 1:
        return frappe.db.sql(
            """select user_id as 'name' from `tabEmployee` where name in ({0}) """.format(
                ", ".join(frappe.db.escape(i) for i in employees)
            ),
            as_list=1,
        )
...
def get_leave(user, due_date):
    ...
    leave_application = frappe.db.sql(
        """select name from `tabLeave Application` where %s between from_date and to_date and employee=%s and docstatus=1""",
        (due_date, employee),
        as_dict=1,
    )
