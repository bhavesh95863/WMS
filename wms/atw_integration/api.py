...
def get_batch_list(employee):
	try:
		...
		batches = frappe.db.sql("""select distinct batch_no as 'batch_no','' as 'portion_length' from `tabPortion Traceability` where current_warehouse=%s and target_warehouse is not null""",warehouse,as_dict=1)
		for batch in batches:
			batch.portion_length = len(frappe.get_all("Portion Traceability",filters=[["Portion Traceability","batch_no","=",batch.batch_no],["Portion Traceability","current_warehouse","=",warehouse],["Portion Traceability","target_warehouse","is","set"]],fields=["name"]))
...
def get_portion_list(employee,batch_no):
	try:
		...
		if not warehouse:
			return gen_response(200, "No any linked warehouse with loggedin user",[])
		portions = frappe.db.sql("""select distinct batch_no as 'batch_no',portion_no as 'portion_no',welding as 'welding',welding_tolerance 'tolerance',welding as 'welding_sync',welding_tolerance 'tolerance_tolerance' from `tabPortion Traceability` where current_warehouse=%s and batch_no=%s and target_warehouse is not null""",(warehouse,batch_no),as_dict=1)
