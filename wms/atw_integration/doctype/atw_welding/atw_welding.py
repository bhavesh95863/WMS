...
class ATWWelding(Document):
	def autoname(self):
		if self.get('auto_gen_id'):
			...
			self.name = cstr(getdate(today()).month) + '-' + cstr(getdate(today()).year) + '-' + self.welder_code + '-' + self.weld_no

	def after_insert(self):
		portion_doc = frappe.get_doc("Portion Traceability",self.portion)
		portion_doc.actual_welder = self.actual_welder
		portion_doc.weld_no = self.weld_no
	...
	def on_update(self):
		if self.tolerance:
			...
			portion_doc.tolerance_1_mtr_horizontal = self.get("tolerance_1_mtr_horizontal")
			portion_doc.tolerance_10cm_horizontal = self.get("tolerance_10cm_horizontal")
