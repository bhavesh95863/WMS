...
class USFDTestResult(Document):
	...
	def validate_before_submit(self):
		for row in self.details:
			if not row.good and not row.defective:
				frappe.throw(_("Row{0}: Select Good or Defective Before Submit".format(row.idx)))
	...
	def populate_data(self):
		...
		if self.testing == "Internal":
			filters.append(["Portion Traceability","internal_usfd_result","is","not set"])
		if self.testing == "External":
			filters.append(["Portion Traceability","external_usfd_result","is","not set"])
		portion_trace_details = frappe.get_all("Portion Traceability",filters=filters,fields=["weld_no","date_of_welding","km_tp","lh_rh","name"])
...
def get_site_warehouse(doctype, txt, searchfield, start, page_len, filters):
	return frappe.db.sql("""select target_warehouse as 'name' from `tabPortion Traceability` where target_warehouse is not null""",as_list=1)
